...
Classical N-body problem describes motion of N point masses in their mutual gravitational field. Each body at some time $t$ has mass $m_i$, position $\textbf{r}_i$, and velocity $\textbf{v}_i$. Hence, the Hamiltonian of the system is given as:

\begin{equation}
	H = \sum_{i=1}^{N} \frac{\textbf{p}_i^2}{2m_i} - \sum_{i=1}^{N} \sum_{j=i+1}^{N} \frac{Gm_i m_j}{|\textbf{q}_i - \textbf{q}_j|}
\end{equation}

with canonical momentum and position as $\textbf{p}_i = m_i \textbf{v}_i$ and $\textbf{q}_i = \textbf{r}_i$ respectively.

The equation of motion is calculated from the Hamiltonian as:
